import React, {Component} from 'react';
import {connect} from 'react-redux'

import Notifications from './Notifications'
import SuggestionList from '../Suggestions/SuggestionList'
import {getSuggestions, clearSuggestions, addSuggestionFromSSE} from '../../Store/Actions/SuggestionActions';
import {AuthState} from '../../Store/Constants/UserConstants'

import ServerService from '../../Services/ServerService';

class Dashboard extends Component{


    componentDidMount(){
        //announce redux it needs a state update
        if(this.props.authState === AuthState.TRUE){
            this.props.getSuggestion();
            ServerService.subscribe('UPDATE_STORE', (suggestion)=>{this.props.addSuggestionFromSSE(suggestion); console.log(suggestion) }, 'addsuggestion');
        }
        else{
            this.props.history.push('/signin');
        }
    }

    componentWillUnmount(){
        ServerService.unsubscribe('addsuggestion');
    }

    componentDidUpdate(){
        if(this.props.authState !== AuthState.TRUE){
            this.props.clearSuggestion();
            this.props.history.push('/signin');
        }
    }

    render(){

        const {suggestions}=this.props;

        return(
           <div className='dashboard container'>
            <div className='row'>

                <div className='col s12 m6'>
                    <SuggestionList suggestions={suggestions}/>
                </div>      

                <div className='col s12 m5 offset-m1'>
                    <Notifications />
                </div>       
    
            </div>
           </div> 
        )
    }
}

const mapStateToProps = (state) =>{
    return{
        suggestions: state.suggestion.suggestions,
        authState: state.auth.authState
    }
}

const mapDispatchToProps = (dispatch) =>{
    return{
        getSuggestion: () => dispatch(getSuggestions()),
        clearSuggestion: () => dispatch(clearSuggestions()),
        addSuggestionFromSSE: (suggestion) => dispatch(addSuggestionFromSSE(suggestion))
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(Dashboard);